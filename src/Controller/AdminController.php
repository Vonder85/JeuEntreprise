...
class AdminController extends AbstractController
{
    ...
    function affectationPoule($idEvent, Request $request, EntityManagerInterface $em)
    {
        ...
        if ($event->getRound()->getName() === "Tournoi principal" && $event->getPhase() ===3 && sizeof($participations) == 6){
    ...
    function creerRencontresDemiFinale($idEvent, EntityManagerInterface $em, Request $request)
    {
        ...
        if(sizeof($participations) === 8 && ($event->getRound()->getName() == "1/4 finale consolante" ||$event->getRound()->getName() == "1/4 finale") && (sizeof($participationsDebut) === 16 || sizeof($participationsDebut) === 18 || sizeof($participationsDebut) === 19 || sizeof($participationsDebut) === 20 || sizeof($participationsDebut) === 21)){
    ...
    function creerPhasesFinale($idEvent, EntityManagerInterface $em, Request $request)
    {
       ...
       if(($totalParticipationsDebut === 16 || $totalParticipationsDebut === 17 || $totalParticipationsDebut === 15 || $totalParticipationsDebut === 18 || $totalParticipationsDebut === 19 || $totalParticipationsDebut === 20 || $totalParticipationsDebut === 21) && ($event->getRound()->getName() == "1/4 finale consolante" || $event->getRound()->getName() == "1/2 finale consolante")) {
            ...
            if($event->getRound()->getName() == "1/4 finale consolante" && $totalParticipationsDebut === 15){
                $eventPhase1 = $em->getRepository(Event::class)->findEventWithEventNameRoundNameAndCompetition($event->getName(), $roundPhase1, $event->getCompetition());
    ...
    /**
     * fonction qui crée les participations 3/5/7 emeplace a partir de 15equipes
     * @Route("/rencontre357emeplace/{idEvent}", name="creation_rencontres_357emeplace", requirements={"idEvent": "\d+"})
     */
     ...
     */
    public
    function creerQuartFinales18equipes($idEvent, EntityManagerInterface $em, ParticipationRepository $pr, Request $request)
    {
        $event = $em->getRepository(Event::class)->find($idEvent);
        $roundName = $request->request->get('round');
        $round = $em->getRepository(Round::class)->findOneBy(["name" => $roundName]);
     ...
     */
    public function pouleClassement19Equipes($idEvent, EntityManagerInterface $em, Request $request, ParticipationRepository $pr){
        $event = $em->getRepository(Event::class)->find($idEvent);
        $roundName = $request->request->get('round');
        $round = $em->getRepository(Round::class)->findOneBy(["name" => $roundName]);
        $matchs = $em->getRepository(Match::class)->findMatchesWithAnEvent($event);
     ...
     */
    public
    function creerQuartAPartir15equipes($idEvent, EntityManagerInterface $em, Request $request)
    {
        $event = $em->getRepository(Event::class)->find($idEvent);
        $roundName = $request->request->get('round');
     ...
     */
    public function tournoiConsolante3phases($idEvent, EntityManagerInterface $em, Request $request, ParticipationRepository $pr){
        $event = $em->getRepository(Event::class)->find($idEvent);
        $roundName = $request->request->get('round');
        $round = $em->getRepository(Round::class)->findOneBy(["name" => $roundName]);
        $participations = $pr->findParticipationInAnEventSimple($idEvent);
        ...
        }elseif(sizeof($participations) == 8){
            $participations = RencontreUtils::participations3emeet4eme($participationsPoule,$event1);
     ...
     */
    public function tournoiPrincipal3phases($idEvent, EntityManagerInterface $em, Request $request, ParticipationRepository $pr){
        $event = $em->getRepository(Event::class)->find($idEvent);
        $roundName = $request->request->get('round');
        $round = $em->getRepository(Round::class)->findOneBy(["name" => $roundName]);
        $participations = $pr->findParticipationInAnEventSimple($idEvent);
        ...
        if(sizeof($participations) == 7 || sizeof($participations) == 8){
     ...
     * @Route("/creerPlace357FinalePhase3/{idEvent}", name="places_357finale_phase3", requirements={"idEvent": "\d+"})
     */
    public function places357finalePhase3($idEvent, EntityManagerInterface $em, Request $request, ParticipationRepository $pr){
        $event = $em->getRepository(Event::class)->find($idEvent);
        $roundName = $request->request->get('round');
        $round = $em->getRepository(Round::class)->findOneBy(["name" => $roundName]);
     ...
     */
    public function placesHonneurPhase3($idEvent, EntityManagerInterface $em, Request $request, ParticipationRepository $pr){
        $event = $em->getRepository(Event::class)->find($idEvent);
        $roundName = $request->request->get('round');
        $round = $em->getRepository(Round::class)->findOneBy(["name" => $roundName]);
